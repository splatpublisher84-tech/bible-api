...
const titles = { overview: 'Overview', traffic: 'Traffic', logs: 'Logs', costs: 'Costs & System', endpoints: 'API Reference' };
...
function renderTrafficChart(hourly) {
  ...
  const data = {
    labels,
    datasets: [
      { label: 'Client', data: hourly.map(h => h.client), backgroundColor: '#3b82f6', borderRadius: 2 },
      { label: 'Internal', data: hourly.map(h => h.internal), backgroundColor: '#475569', borderRadius: 2 },
      { label: 'Errors', data: hourly.map(h => h.errors), backgroundColor: '#ef4444', borderRadius: 2 },
    ],
  };
  const opts = {
  ...
  if (trafficChart) { trafficChart.data = data; trafficChart.update(); }
  else { trafficChart = new Chart(ctx, { type: 'bar', data, options: opts }); }
}
...
function renderResponseTimeChart(hourly) {
  ...
  const data = {
    labels,
    datasets: [
      { label: 'p50', data: hourly.map(h => h.p50 ?? 0), borderColor: '#22c55e', backgroundColor: '#22c55e22', tension: 0.3, pointRadius: 2, fill: false },
      { label: 'p95', data: hourly.map(h => h.p95 ?? 0), borderColor: '#f59e0b', backgroundColor: '#f59e0b22', tension: 0.3, pointRadius: 2, fill: false },
      { label: 'p99', data: hourly.map(h => h.p99 ?? 0), borderColor: '#ef4444', backgroundColor: '#ef444422', tension: 0.3, pointRadius: 2, fill: false },
    ],
  };
  const opts = {
    responsive: true, maintainAspectRatio: false,
    ...
    plugins: {
      ...
      tooltip: { mode: 'index', intersect: false, callbacks: { label: ctx => ctx.dataset.label + ': ' + ctx.parsed.y + 'ms' } },
    },
  };

  if (responseTimeChart) { responseTimeChart.data = data; responseTimeChart.update(); }
  else { responseTimeChart = new Chart(ctx, { type: 'line', data, options: opts }); }
}
...
function renderErrorRateChart(hourly) {
  ...
  const data = {
    labels,
    datasets: [
      { label: '4xx', data: hourly.map(h => h.errors_4xx ?? 0), borderColor: '#fbbf24', backgroundColor: '#fbbf2422', tension: 0.3, pointRadius: 2, fill: true },
      { label: '5xx', data: hourly.map(h => h.errors_5xx ?? 0), borderColor: '#ef4444', backgroundColor: '#ef444433', tension: 0.3, pointRadius: 2, fill: true },
    ],
  };
  const opts = {
    responsive: true, maintainAspectRatio: false,
...
function renderHealthTimeline(health) {
  ...
  const slots = health.map(h => {
    const cls = h.status === 'ok' ? 'health-ok' : h.status === 'degraded' ? 'health-degraded' : h.status === 'down' ? 'health-down' : 'health-unknown';
  ...
  container.innerHTML =
    '<div class="health-timeline">' + slots + '</div>' +
    '<div class="health-labels"><span>' + esc(first) + '</span><span>' + esc(last) + '</span></div>';
}
    ...
    el.innerHTML = endpoints.map(e => {
      ...
      return '<div style="padding:6px 0;border-bottom:1px solid #1e293b;">' +
        '<div style="display:flex;justify-content:space-between;margin-bottom:4px;">' +
          '<span class="ep-name">' + esc(e.endpoint) + '</span>' +
          '<span style="font-size:12px;font-weight:600;">' + e.total + '</span>' +
        '</div>' +
        '<div class="ep-bar">' +
          '<div class="ep-bar-client" style="width:' + clientW + '%"></div>' +
          '<div class="ep-bar-internal" style="width:' + internalW + '%"></div>' +
        '</div></div>';
    }).join('');
    return;
  }
  const tbody = endpoints.map(e => {
    const maxTotal = endpoints[0].total;
    const clientW = maxTotal > 0 ? (e.client / maxTotal) * 100 : 0;
    const internalW = maxTotal > 0 ? (e.internal / maxTotal) * 100 : 0;
    return '<tr><td class="ep-name">' + esc(e.endpoint) + '</td>' +
      '<td style="color:#3b82f6">' + e.client + '</td>' +
      '<td style="color:#64748b">' + e.internal + '</td>' +
      '<td style="font-weight:600">' + e.total + '</td>' +
      '<td><div class="ep-bar"><div class="ep-bar-client" style="width:' + clientW + '%"></div><div class="ep-bar-internal" style="width:' + internalW + '%"></div></div></td></tr>';
  }).join('');
...
async function loadStatus() {
  try {
    ...
    document.getElementById('apiDot').className = 'status-dot ' + (d.api.status === 'ok' ? 'status-ok' : 'status-error');
    ...
    document.getElementById('dbDot').className = 'status-dot ' + (d.database.status === 'ok' ? 'status-ok' : 'status-error');
    document.getElementById('dbLatency').textContent = d.database.latency ? 'Latency: ' + d.database.latency : d.database.message || '';
    ...
    if (d.costs) {
      ...
      document.getElementById('dbLimit').textContent = s.db_usage_percent + '% of ' + s.db_limit + ' free tier';
      document.getElementById('dbProgress').style.width = s.db_usage_percent + '%';
      document.getElementById('dbProgress').className = 'progress-fill ' + (s.db_usage_percent >= 80 ? 'progress-danger' : s.db_usage_percent >= 50 ? 'progress-warn' : 'progress-ok');
      document.getElementById('supabaseTier').textContent = s.tier;

      document.getElementById('dbSizeOverview').textContent = s.db_size;
      document.getElementById('dbLimitOverview').textContent = s.db_usage_percent + '% of ' + s.db_limit;
      document.getElementById('dbProgressOverview').style.width = s.db_usage_percent + '%';
      document.getElementById('dbProgressOverview').className = 'progress-fill ' + (s.db_usage_percent >= 80 ? 'progress-danger' : s.db_usage_percent >= 50 ? 'progress-warn' : 'progress-ok');
...
async function loadMetrics() {
  try {
    ...
    document.getElementById('clientRequestsDetail').textContent = m.totals.total.toLocaleString() + ' total (' + m.totals.internal + ' internal)';
